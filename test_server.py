...
async def generate_audio_bytes(text, voice, language, temperature, repetition_penalty, speed, pitch, output_file, streaming=False):
    """Generate TTS audio with specified parameters. Supports streaming and non-streaming modes."""
    debug_func_entry()
    ...
    response = await model_engine.generate_tts_bytes(text, voice, language, temperature, repetition_penalty, speed, pitch, output_file, streaming)
...
async def tts_process_standard_mode_bytes(params: dict, text_input: str) -> Union[StreamingResponse, Tuple[Path, str, str]]:
    """Process text with standard mode, without narrator functionality."""
    debug_func_entry()
    ...
    response = await generate_audio_bytes(
                cleaned_text, params['character_voice_gen'], params['language'],
                params['temperature'], params['repetition_penalty'],
                params['speed'], params['pitch'],
                output_file_path, True
...
@app.post("/api/tts-generate-bytes", response_class=StreamingResponse)
async def apifunction_generate_tts_standard(
    text_input: str = Form(...),
    text_filtering: str = Form(None),
    character_voice_gen: str = Form(None),
    ...
    try:
        ...
        output = await tts_process_standard_mode_bytes(
                    params, text_input
                )
        buff = io.BytesIO()
        print("before save")

        torchaudio.save(buff, torch.tensor(output["wav"]).unsqueeze(0), 24000, format="wav")
        buff.seek(0)
        print("after save")
        headers = {'Content-Disposition': 'attachment; filename="out.wav"'}
        
        return StreamingResponse(buff, media_type="audio/wav",
    headers={'Content-Disposition': 'inline; filename="out.wav"'})
        #return Response(buff, headers=headers, media_type='audio/wav')
        # Process based on mode
    except ValueError as e:
        print_message(f"Invalid parameter value: {str(e)}", "error", "GEN")
        return JSONResponse(content={"status": "generate-failure", "error": str(e)}, status_code=400)
